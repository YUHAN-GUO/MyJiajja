...
import com.base.gyh.baselib.widgets.vptran.GalleryTransformer;
...
public class ViewPagerGallery extends ViewPager {
    ...
    private void init() {
//        int pagerWidth = (int) (mContext.getResources().getDisplayMetrics().widthPixels * 3.0f / 10.0f);
    ...
    public void setImgUrls(List<String> imgUrls) {
        ...
        for (int i = 0; i < imgUrls.size(); i++) {
            ...
            if (isShadow) {
                Glide.with(mContext).asBitmap().load(url).centerCrop().into(new SimpleTarget<Bitmap>() {
                    @Override
        ...
        this.setAdapter(new MyViewPagerAdapter(views));
        init();
    ...
    public void setImgResources(List<Integer> imgResources) {
        ...
        for (int i = 0; i < imgResources.size(); i++) {
            ...
            if (true){
                Glide.with(mContext).asBitmap().load(id).into(new BitmapImageViewTarget(imageView){
                    @Override
                    public void onResourceReady(@NonNull Bitmap resource, @Nullable Transition<? super Bitmap> transition) {
