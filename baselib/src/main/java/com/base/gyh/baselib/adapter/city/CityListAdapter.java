package com.base.gyh.baselib.adapter.city;
...
import com.base.gyh.baselib.adapter.decoration.GridItemDecoration;
...
import static com.base.gyh.baselib.adapter.city.GridListAdapter.SPAN_COUNT;
...
public class CityListAdapter extends RecyclerView.Adapter<CityListAdapter.BaseViewHolder> {
    ...
    private static final CityPicker.LocateState LocateState = new CityPicker.LocateState();

    private Context mContext;
    private List<City> mData;
    private List<HotCity> mHotData;
    private int locateState;
    private CityPicker.InnerListener mInnerListener;
    ...
    public void onBindViewHolder(BaseViewHolder holder, int position) {
        ...
        if (holder instanceof LocationViewHolder) {
            ...
            ((LocationViewHolder) holder).container.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (locateState == CityPicker.LocateState.SUCCESS) {
                        if (mInnerListener != null) {
                            mInnerListener.dismiss(pos, data);
                        }
                    } else if (locateState == CityPicker.LocateState.FAILURE) {
                        locateState = CityPicker.LocateState.LOCATING;
                        notifyItemChanged(0);
                        if (mInnerListener != null) {
    ...
    public void setInnerListener(CityPicker.InnerListener listener) {
    ...
    public static class HotViewHolder extends BaseViewHolder {
        ...
        HotViewHolder(View itemView) {
            ...
            mRecyclerView.setLayoutManager(new GridLayoutManager(itemView.getContext(),
                    SPAN_COUNT, LinearLayoutManager.VERTICAL, false));
            int space = itemView.getContext().getResources().getDimensionPixelSize(R.dimen.cp_grid_item_space);
            mRecyclerView.addItemDecoration(new GridItemDecoration(SPAN_COUNT,
                    space));
        }
    }
