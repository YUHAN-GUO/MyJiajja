    ...
    public void enqueue(final ResultMyCall resultMyCall) {
        if (onlyOneNet) {
            if (!TextUtils.isEmpty(tag)) {
                if (OkHttpUtils.getInstance().getOnesTag().contains(tag)) {
                    return;
                }
                OkHttpUtils.getInstance().getOnesTag().add(tag);
            } else {
                if (OkHttpUtils.getInstance().getOnesTag().contains(url)) {
                    return;
                }
                OkHttpUtils.getInstance().getOnesTag().add(url);
            }
        }
...
//                            }
                ...
                if (response.isSuccessful()) {
                    ...
                    if (resultMyCall != null) {
                        ...
                        mDelivery.post(new Runnable() {
                            ...
                            public void run() {
                                resultMyCall.onAfter();
                                resultMyCall.onError(new ClassCastException(errorMsg));
                            }
                        });
                    }
                }
    ...
    public void enqueue(final ResultCall resultMyCall) {
        ...
        if (params != null) {
            Logger.d("%s+++++++++++%s,","guoyh","请求参数 ==> " + GsonUtil.ser(params));
        ...
        RequestBody requestBodyProgress = new CountingRequestBody(requestBody, new CountingRequestBody.Listener() {
            ...
            public void onRequestProgress(final long bytesWritten, final long contentLength) {
                mDelivery.post(new Runnable() {
                    ...
                    public void run() {
                        resultMyCall.inProgress(bytesWritten * 1.0f / contentLength);
                        Logger.d("%s+++++++++++%s,","guoyh","请求进度 ==> " + bytesWritten * 1.0f / contentLength);
                    }
                });
                    ...
                    mDelivery.postDelayed(new Runnable() {
                        ...
                        public void run() {
                            ...
                            if (e instanceof SocketException) {
                                ...
                                Logger.d("%s+++++++++++%s,","guoyh","请求失败原因 ==> " + e.toString());
    ...
    public void removeOnceTag() {
        if (onlyOneNet) {
            if (!TextUtils.isEmpty(tag)) {
                OkHttpUtils.getInstance().getOnesTag().remove(tag);
            } else {
                OkHttpUtils.getInstance().getOnesTag().remove(url);
            }
        }
    }
